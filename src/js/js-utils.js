/*DOM utils*/
export function addExpandCollapseBehavior(buttonElement, toggleClassesObj, toggleHandler) {
   const regionElement = document.getElementById(buttonElement.getAttribute('aria-controls'));
   //binding events;
   ...
   buttonElement.addEventListener('keydown', (e) => {
      if(e.key === "Control" || e.key === "Enter") {//as key numbers are being depricated
          toggleHandler(buttonElement, regionElement, toggleClassesObj);
          e.stopPropagation();
          return false;
      }
...
function expandCollapseToggleHandler(buttonElement, regionElement,toggleClassObj){
